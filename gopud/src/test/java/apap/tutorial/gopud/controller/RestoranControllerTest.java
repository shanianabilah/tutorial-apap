...
public class RestoranControllerTest {
    ...
    @Test
    public void whenRestoranViewItShouldSuccesfullReturnToRightView() throws Exception{
        RestoranModel resto = generateDummyRestoranModel(1);
        when(restoranService.getRestoranByIdRestoran((long)1)).thenReturn(Optional.of(resto));
        mockMvc.perform(get("/restoran/view?idRestoran=1")).andExpect(MockMvcResultMatchers.status().isOk())
                .andExpect(content().string(Matchers.containsString("Informasi Restoran")))
                .andExpect(content().string(Matchers.containsString("ID Restoran")))
                .andExpect(model().attribute("resto", is(resto)));
